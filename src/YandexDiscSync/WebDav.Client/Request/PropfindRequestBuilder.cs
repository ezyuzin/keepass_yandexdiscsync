...
namespace WebDav
{
    internal static class PropfindRequestBuilder
    {
        ...
        private static string BuildAllPropRequest(List<XName> customProperties, List<NamespaceAttr> namespaces)
        {
        ...
        private static string BuildNamedPropRequest(List<XName> customProperties, List<NamespaceAttr> namespaces)
        {
            var doc = new XDocument(new XDeclaration("1.0", "utf-8", null));
            var propfind = new XElement("{DAV:}propfind", new XAttribute(XNamespace.Xmlns + "D", "DAV:"));
            if (customProperties.Any())
