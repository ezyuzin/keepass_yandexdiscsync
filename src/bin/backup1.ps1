...
function backup($dir) {
  ...
  if (test-path "$backupDir/state.json") {
    foreach($line in [IO.File]::ReadLines("$backupDir/state.json")) {
  ...
  $modified = 0;
  if (test-path "$backupDir/modified.txt") {
    remove-item "$backupDir/modified.txt";
  }

  foreach($name in $state.Keys) {
    if (($prev.ContainsKey($name) -eq $false) -or ($prev[$name]['timestamp'] -ne $state[$name]['timestamp']) -or ($prev[$name]['length'] -ne $state[$name]['length'])) {
      "$name" | Out-File -Append -LiteralPath "$backupDir/modified.txt";
      $modified += 1;
    }
  }
  
  if ($modified -ne 0) {
    &"$7zip" a "$backupDir/$backupName.7z" -y -mx9 -t7z -scsUTF-8 -bb3 "@$backupDir/modified.txt"
    remove-item "$backupDir/modified.txt"
  }

  if (($deleted + $modified) -ne 0) {
    "" | Out-File -NoNewline -LiteralPath "$backupDir/state.json";
    foreach($name in $state.Keys) {
      (ConvertTo-Json -Compress -InputObject $state[$name]) | Out-File -Append -LiteralPath "$backupDir/state.json"
    }
